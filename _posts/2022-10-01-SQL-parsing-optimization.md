...
---
SQL 튜닝을 본격적으로 시작하기에 앞서 옵티마이저가 SQL을 어떻게 처리하는지

서버 프로세스는 데이터를 어떻게 읽고 저장하는지 이번 포스트에서 살펴보려고 한다.

DW 설계를 마치고 본격적으로 DW 구축을 시작하기 전에 미리 튜닝의 개념을 잡아놓고 싶었다. 앞으로 쿼리를 짤 때 어떤 관점으로 쿼리를 바라봐야하는지, 개인적으로 공부하면 도움이 될거같아서 시작한 스터디다. 

마무리가 어떻게 될진 모르겠지만! 그래도 일단 SQL 튜닝의 세계로 여행을 떠나볼거다!
...
요즘에는 위와 같이 SQL로 간단히 처리 할 수 있었지만 20 ~ 30년 전만해도 이런 로직을 어렵게 프로그래밍으로 풀어냈다고 한다..

## SQL이란?

Structured Query Language의 줄임말이며 말 그대로 `구조적 질의 언어`다. 위키피디아에서는 아래와 같이 정의하기도 한다:

> SQL is **set-based, declartive query lanuage**, not an imperitive language such as C or BASIC.

원하는 결과집합을 구조적, 집합적으로 선언하지만 그 결과 집합을 만드는 과정은 절차적일 수 밖에 없다. 즉, 프로시저(Procedure)가 필요한데 그런 **프로시저를 만들어내는 DBMS 내부 엔진이 바로 SQL 옵티마이저다**. 옵티마이저가 프로그래밍을 대신해 주는 셈이다.
...
SQL 옵티마이저가 선택한 실행경로를 실제 실행 가능한 코드 또는 프로시저 형태로 **포맷팅하는 단계**다. Row Source Generator가 그 역할을 맡다.

<br>

# SQL 옵티마이저

SQL 옵티마이저는 사용자가 원하는 작업을 가장 효율적으로 수행할 수 있는 **최적의 데이터 액세스 경로를 선택해 주는 DBMS의 핵심**다. 옵티마이저의 최적화 단계를 요약하면 아래와 같다.
...
**옵티마이저는 자동차 네비게이션과 흡사한다**. 경로를 검색하고 나서 이동 경로를 미리 확인하는 기능이 있기 때문에 선택한 경로가 마음에 들지 않으면 이동 경로를 변경하거나, 경유지를 추가 해서 운전자가 원하는 경로로 바꿀 수 있다.

위와 같은 기능을 'SQL 실행경로 미리 보기'로 확인 할 수 있으며, **실행 계획(Execution Plan)**이라 한다. 정확히 말하면 처리절차를 사용자가 확인 할 수 있게 아래와 같이 트리구조로 표현한것이 실행 계획이다.
...
DBMS 엔진은 요청받은 쿼리문을 실행하기 위해 파싱(Parsing)이라는 과정을 진행한다. 이 파싱에는 문법 체크, 권한 유무, Library cache에 기존 쿼리가 있는지를 체크하는 공통 파싱 그리고 Library chace에 기존 쿼리가 있는 경우 진행하는 소프트 파싱(Soft parsing), 기존 쿼리가 없는 경우 진행하는 하드 파싱(Hard parsing) 총 3가지가 있다. 파싱을 설명하기 앞서 오라클 DB의 SGA에 대해 설명하겠다.

오라클에는 기본적으로 메모리 영역과 (SGA) / 데이터베이스 영역으로 나뉘어져 있다. 여기서 SGA (System Global Area)란, 서버 프로세스와 백그라운드 프로세스가 공통으로 액세스하는 데이터와 제어 구조를 캐싱하는 메모리 공간을 말한다.

![Desktop View](/230413/1-4-1.jpeg){: width="600" height="400"}

SQL을 수행 할 때 실행 계획이 바로 이 Library Cache 영역에 저장되어 사용된다.

## 라이브러리 캐시(Library Cache)

SQL 파싱, 최적화, 로우 소스 생성 과정을 거쳐 생성한 내부 프로시저를 반복 재사용할 수 있도록 캐싱해 두는 메모리 공간을 말한다.
...
SQL 을 캐시에서 찾아 곧바로 실행단계로 넘어가는 것을 말한다.
...
SQL 을 캐시에서 찾지 못하여 최적화 및 로우 소스 생성 단계를 모두 거치는 것을 말한다.
...
일단 단도직입적으로 말하면 **디스크 I/O 때문다.** 디스크 I/O가 발생한다면 어떤 일들이 벌어질까요? 우선 OS 또는 I/O 서브 시스템이 I/O를 처리하는 동안 프로세스는 잠을 자게되고, **정확히 표현하자면 대기 상태에 빠진다.** 

프로세스가 디스크에서 데이터를 읽어야 할 땐 CPU를 OS에 반환하고 잠시 수면(Waiting) 상태에서 I/O가 완료되기를 기다린다. 따라서 OS가 IO를 처리중이면 실행중인 프로세스는 interrupt로 인해 수시로 실행 준비 상태 ( Runnable Queue )로 전환했다가 다시 실행 상태로 전환하게 된다. 이런 이유 때문에 I/O가 많으면 성능이 저하 될 수 밖에 없으며, **우리의 역할은 I/O의 횟수를 최소화 하는것이다.**

> I/O 튜닝이 곧 SQL 튜닝이라고 할 수 있다.
...
데이터 블록은 자신만의 고유 주소값을 의미한다. 즉, 디스크 상에서 몇 번 데이터파일의 몇 번째 블록인지를 나타낸다.
인덱스를 이용해 테이블 레코드를 읽을 대는 인덱스 ROWID를 이용해야하며, ROWID 는 DBA + 로우번호(블록 내 순번, 로케이션)으로 구성되어 있다.
...
테이블 또는 인덱스 블록을 액세스 하는 방식은 2가지가 있다.
...
읽은 전체 블록 중에서 물리적인 디스크 I/O를 수반하지 않고 곧바로 메모리에서 찾는 비율을 말한다.
...
한 번에 한 블록씩 요청해서 메모리에 적재하는 방법이다.
...
한 번에 여러 블록씩 요청해서 메모리에 적재하는 방법이다.
...
테이블 전체를 스캔하는 방식과 인덱스를 이용한 테이블 액세스 방식의 차이를 알아보겠다.
...
버퍼 캐시에서 해시 체인을 탐색하면서 대량의 데이터를 읽는데, 그사이에 체인이 수정되면 안된다. 따라서 해시체인 래치가 존재하고 키를 획득한 프로세스만이 진입 가능하다.
